...
class CultivationServiceTest {
  ...
  @Mock private DaoProtectionRepository daoProtectionRepository;
  ...
  void establishProtection_whenAlreadyProtecting_shouldReturnFailure() {
    ...
    when(daoProtectionRepository.findByProtectorAndProtege(userId, 2L))
        .thenReturn(Optional.of(DaoProtection.create().setProtectorId(userId).setProtegeId(2L)));
  ...
  void removeProtection_whenNotProtecting_shouldReturnFailure() {
    when(userRepository.findByNickname("弟子"))
        .thenReturn(Optional.of(User.create().setId(2L).setNickname("弟子")));
    when(daoProtectionRepository.findByProtectorAndProtege(userId, 2L))
        .thenReturn(Optional.empty());
