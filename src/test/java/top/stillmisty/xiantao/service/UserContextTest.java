...
class UserContextTest {
  ...
  void getCurrentUserId_whenBound_shouldReturnValue() throws Exception {
    Long result =
        ScopedValue.where(UserContext.CURRENT_USER, 42L).call(() -> UserContext.getCurrentUserId());
  ...
  void getCurrentUserId_afterScope_shouldNotBeBound() throws Exception {
    Long result =
        ScopedValue.where(UserContext.CURRENT_USER, 99L).call(() -> UserContext.getCurrentUserId());
