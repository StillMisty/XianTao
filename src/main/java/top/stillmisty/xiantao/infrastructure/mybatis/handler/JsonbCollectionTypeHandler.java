...
/**
 * PostgreSQL JSONB 集合类型处理器，专用于 List/Set 字段。 Jackson 会根据 propertyType（List/Set
 * 接口）自动选择对应的实现类（ArrayList/HashSet）。
 */
public class JsonbCollectionTypeHandler extends BaseTypeHandler<Object> {
  ...
  public JsonbCollectionTypeHandler(Class<?> propertyType, Class<?> genericType) {
    this.propertyType = propertyType;
  ...
  public void setNonNullParameter(PreparedStatement ps, int i, Object parameter, JdbcType jdbcType)
    ...
    try {
      ...
      throw new SQLException("Failed to serialize collection to JSONB", e);
    }
  }
