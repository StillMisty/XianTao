...
public interface PlayerBuffMapper extends BaseMapper<PlayerBuff> {
    ...
    @Select("SELECT * FROM xt_player_buff WHERE user_id = #{userId} AND buff_type = #{buffType} AND expires_at > NOW()")
    List<PlayerBuff> selectActiveByUserIdAndType(@Param("userId") Long userId, @Param("buffType") String buffType);
