...
public enum Rarity {
     ...
     * 前缀示例: 破烂的、生锈的
     */
    BROKEN("broken", "破旧", RarityColor.WHITE, 0.8, 0.99, 0, 0,
     ...
     * 前缀示例: 普通的、结实的
     */
    COMMON("common", "普通", RarityColor.GREEN, 1.0, 1.15, 1, 1,
     ...
     * 前缀示例: 锋利的、精工的
     */
    RARE("rare", "稀有", RarityColor.BLUE, 1.16, 1.35, 1, 2,
     ...
     * 前缀示例: 卓越的、附魔的
     */
    EPIC("epic", "史诗", RarityColor.PURPLE, 1.36, 1.60, 2, 3,
     ...
     * 前缀示例: 传说的、完美的
     */
    LEGENDARY("legendary", "传说", RarityColor.GOLD, 1.61, 2.00, 3, 4,
    ...
    Rarity(String code, String name, RarityColor color, double qualityMultiplierMin, double qualityMultiplierMax,
    ...
    public String getRandomPrefix() {
        ...
        return prefixes.get((int) (Math.random() * prefixes.size()));
    }

    /**
     * 获取随机品质系数
     */
    public double getRandomQualityMultiplier() {
        double range = qualityMultiplierMax - qualityMultiplierMin;
        return qualityMultiplierMin + Math.random() * range;
    }
    ...
    public int getRandomAffixCount() {
        if (affixCountMin == affixCountMax) {
            return affixCountMin;
        }
        return affixCountMin + (int) (Math.random() * (affixCountMax - affixCountMin + 1));
    }
