...
public enum BeastQuality {
  MORTAL("MORTAL", "凡品", 0, 1.0, 1.0, 1.0, 600, 30),
  SPIRIT("SPIRIT", "灵品", 1, 1.2, 0.85, 1.1, 250, 60),
  IMMORTAL("IMMORTAL", "仙品", 2, 1.5, 0.70, 1.2, 100, 120),
  SAINT("SAINT", "圣品", 3, 2.0, 0.55, 1.3, 40, 240),
  DIVINE("DIVINE", "神品", 4, 3.0, 0.40, 1.5, 10, 480);
  ...
  public static BeastQuality fromRank(int rank) {
    for (BeastQuality q : values()) {
      if (q.rank == rank) return q;
    }
    throw new IllegalArgumentException("Unknown BeastQuality rank: " + rank);
  }
}
