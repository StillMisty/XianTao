...
public sealed interface CellConfig {
    ...
    record PenConfig(
        ...
        public PenConfig withProductionStored(List<ProductionItem> productionStored) {
            return new PenConfig(beastId, templateId, hatchTime, matureTime, productionStored, lastProductionTime);
        }
        ...
        public void addProductionItem(Long templateId, String name, int quantity) {
            for (ProductionItem item : productionStored) {
                if (item.templateId().equals(templateId)) {
                    ...
                    productionStored.set(idx, new ProductionItem(templateId, name, item.quantity() + quantity));
                    return;
                }
            }
        ...
        public PenConfig withClearedProduction() {
            return new PenConfig(beastId, templateId, hatchTime, matureTime, new ArrayList<>(), lastProductionTime);
        }
    }
    ...
    class ProductionStoredDeserializer extends JsonDeserializer<List<ProductionItem>> {
        @Override
        public List<ProductionItem> deserialize(JsonParser p, DeserializationContext ctx) throws IOException {
            if (p.isExpectedStartArrayToken()) {
                return ctx.readValue(p, ctx.getTypeFactory().constructCollectionLikeType(List.class, ProductionItem.class));
            }
