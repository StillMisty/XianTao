...
public class BeastHandle {
    ...
    public void deployBeast(MessageEvent event, @FilterValue("position") String position) {
        ...
        String response = beastCommandHandler.handleDeployBeast(
    ...
    public void undeployBeast(MessageEvent event, @FilterValue("position") String position) {
        ...
        String response = beastCommandHandler.handleUndeployBeast(
                PlatformType.ONE_BOT_V11,
                event.getAuthorId().toString(),
                position
        );
    ...
    public void recoverBeast(MessageEvent event, @FilterValue("position") String position) {
        ...
        String response = beastCommandHandler.handleRecoverBeast(
                PlatformType.ONE_BOT_V11,
                event.getAuthorId().toString(),
                position
        );
     ...
     * 处理灵兽进化命令
     * 格式：灵兽进化 [编号] [升阶/升品]
     * 示例：灵兽进化 1 升阶 或 灵兽进化 1 升品
     */
    @Listener
    @ContentTrim
    @Filter("灵兽进化 {{position}} {{mode}}")
    public void evolveBeast(MessageEvent event, @FilterValue("position") String position, @FilterValue("mode") String mode) {
        log.debug("收到灵兽进化请求 - AuthorId: {}, Position: {}, Mode: {}", event.getAuthorId(), position, mode);
        String response = beastCommandHandler.handleEvolveBeast(
                PlatformType.ONE_BOT_V11,
                event.getAuthorId().toString(),
                position,
                mode
    ...
    public void releaseBeast(MessageEvent event, @FilterValue("position") String position) {
        ...
        String response = beastCommandHandler.handleReleaseBeast(
                PlatformType.ONE_BOT_V11,
                event.getAuthorId().toString(),
                position
        );
    ...
    public void getDeployedBeasts(MessageEvent event) {
        ...
        String response = beastCommandHandler.handleGetDeployedBeasts(
                PlatformType.ONE_BOT_V11,
                event.getAuthorId().toString()
        );
        event.replyBlocking(response);
