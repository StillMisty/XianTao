...
public class UseItemHandle {
    ...
    public void useItemWithArgs(MessageEvent event,
                                @FilterValue("itemName") String itemName,
                                @FilterValue("args") String args) {
        log.debug("收到使用物品请求 - AuthorId: {}, ItemName: {}, Args: {}", event.getAuthorId(), itemName, args);
        String response = useItemCommandHandler.handleUseItem(
