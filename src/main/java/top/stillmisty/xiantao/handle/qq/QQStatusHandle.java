...
public class QQStatusHandle {
  ...
  @ContentTrim
  @Filter("状态")
  public void status(QGGroupAtMessageCreateEvent event) {
    log.debug("收到状态查询请求 - AuthorId: {}", event.getAuthorId());
    String response =
        commandHandler.handleStatusMarkdown(PlatformType.QQ, event.getAuthorId().toString());
