...
public class MapService {
  ...
  public List<MapInfoVO> getAllMaps() {
    return mapNodeRepository.findAll().stream().map(this::convertToMapInfoVO).toList();
  }
  ...
  private List<MapInfoVO.MonsterInfoVO> buildMonsterInfoList(
      List<MonsterEncounterEntry> monsterEncounters) {
    if (monsterEncounters == null || monsterEncounters.isEmpty()) {
      return List.of();
    }
    List<Long> templateIds =
        monsterEncounters.stream().map(MonsterEncounterEntry::templateId).toList();
