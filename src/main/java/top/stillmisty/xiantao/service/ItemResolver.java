...
public class ItemResolver {
    ...
    public List<ItemEntry> listSeeds(Long userId) {
        return sortedSeedEntries().apply(userId).stream()
                .map(e -> new ItemEntry(
                        e.index, e.stackable.getTemplateId(), e.stackable.getName(),
                        e.stackable().getQuantity(), tierMetadata(e.template)
                ))
                .toList();
    }
    ...
    public List<ItemEntry> listEquipment(Long userId) {
        return sortedEquipmentEntries(userId).stream()
                .map(e -> new ItemEntry(
                        e.index, e.item.getId(), e.item.getName(),
                        1, e.item.getRarity().getName()
                ))
                .toList();
    }
    ...
    public ResolveResult<Equipment> resolveEquipment(Long userId, String input) {
        ...
        var candidates = partial.stream()
                .map(e -> new ItemEntry(e.index, e.item.getId(), e.item.getName(), 1, e.item.getRarity().getName()))
                .toList();
    ...
    private ResolveResult<ItemTemplate> resolveStackable(List<SeedEntry> entries, String input) {
        ...
        if (partial.isEmpty()) return new NotFound<>(input);
        if (partial.size() == 1) return new Found<>(partial.getFirst().template, partial.getFirst().index);

        var candidates = partial.stream()
                .map(e -> new ItemEntry(
                        e.index, e.template.getId(), e.template.getName(),
                        e.stackable.getQuantity(), tierMetadata(e.template)
                ))
                .toList();
        return new Ambiguous<>(input, candidates);
    ...
    public record Ambiguous<T>(String input, List<ItemEntry> candidates) implements ResolveResult<T> {
    }
