...
public class ExplorationDescriptionFunction {
    ...
    public Response beautify(Request request) {
        log.debug("生成探索描述 - MapName: {}, EventType: {}, FoundItems: {}", request.mapName(), request.eventType(), request.foundItems());
    ...
    private String buildUserMessage(Request request) {
        ...
        sb.append("你深入").append(request.mapName()).append("展开探索，从外围步步推进至深处，").append(request.eventType()).append("。\n");
    ...
    public record Request(
            ...
            @JsonProperty(value = "mapDescription")
            @JsonPropertyDescription("地图描述文本")
            ...
            @JsonProperty(value = "expGained")
            @JsonPropertyDescription("获得的经验值（探索不获取经验时为空）")
            Long expGained,

            @JsonProperty(value = "recipeName")
            @JsonPropertyDescription("发现的配方名称（如有）")
            String recipeName,

            @JsonProperty(value = "eventDescription")
            @JsonPropertyDescription("探索中触发的事件描述（如有）")
