...
/**
 * 用户状态服务
 * 统一入口：加载用户实体并自动解析过期的运行时状态（旅行结算、HP 恢复、buff 清理等）。
 */
@Slf4j
@Service
...
public class UserStateService {
    ...
    /**
     * 加载用户并自动解析过期状态。
     * 替代所有 userRepository.findById(userId).orElseThrow() 调用。
     */
    ...
    private boolean tryCompleteTravel(User user) {
        ...
        log.info("玩家 {} 旅行自动结算：{} → {}", user.getId(), currentMap.get().getName(), destinationMap.get().getName());
    ...
    /**
     * 清理该用户已过期的 buff。
     * SQL 查询已过滤 expires_at > NOW()，此方法清除过期残留以确保一致性。
     */
