...
(1, '青石镇', '青山脚下的小镇，民风淳朴。镇口有座土地庙供往来修士歇脚，是大多数修仙者的起点。',
 'SAFE_TOWN', 1,
 jsonb_build_array(jsonb_build_object('targetId', 2, 'minutes', 3)),
 jsonb_build_array(
...
(2, '翠竹林', '镇北五里的竹海，四季常青。竹林深处偶有低阶妖兽出没，是十里八乡子弟试炼的首选之地。',
 'TRAINING_ZONE', 1,
 jsonb_build_array(jsonb_build_object('targetId', 1, 'minutes', 3), jsonb_build_object('targetId', 3, 'minutes', 5)),
  jsonb_build_array(
...
(3, '青石矿洞', '翠竹林尽头的一个矿洞，洞口已被挖得七零八落。传闻深处有玄铁矿脉，但也盘踞着石灵一类的妖兽。',
 'TRAINING_ZONE', 5,
 jsonb_build_array(jsonb_build_object('targetId', 2, 'minutes', 5), jsonb_build_object('targetId', 4, 'minutes', 8), jsonb_build_object('targetId', 9, 'minutes', 15)),
 jsonb_build_array(
...
(4, '碧水潭', '青石矿洞后方山中的一汪深潭，水色碧绿如玉。潭边常有妖兽饮水，是水属性修炼者的好去处。',
 'TRAINING_ZONE', 10,
 jsonb_build_array(jsonb_build_object('targetId', 3, 'minutes', 8), jsonb_build_object('targetId', 5, 'minutes', 6), jsonb_build_object('targetId', 6, 'minutes', 12)),
 jsonb_build_array(
...
(5, '枫叶坡', '满山遍野的枫树每到秋天便如火焰燃烧，煞是壮观。坡上有一座废弃道观，常有妖物盘踞。',
 'TRAINING_ZONE', 15,
 jsonb_build_array(jsonb_build_object('targetId', 4, 'minutes', 6), jsonb_build_object('targetId', 6, 'minutes', 10), jsonb_build_object('targetId', 7, 'minutes', 8)),
 jsonb_build_array(
...
(6, '云来村', '山谷中的小村，因常年云雾缭绕得名。村中有间茶寮，是往来修士歇脚交换情报的好去处。',
 'SAFE_TOWN', 18,
 jsonb_build_array(jsonb_build_object('targetId', 5, 'minutes', 10), jsonb_build_object('targetId', 4, 'minutes', 12), jsonb_build_object('targetId', 7, 'minutes', 6), jsonb_build_object('targetId', 10, 'minutes', 20)),
 jsonb_build_array(
...
(7, '落日峰', '山峰高耸，黄昏时分夕阳正好从山尖落下，景色绝美。峰腰有一条险陡小径通往峰顶道观。',
 'TRAINING_ZONE', 22,
 jsonb_build_array(jsonb_build_object('targetId', 6, 'minutes', 6), jsonb_build_object('targetId', 5, 'minutes', 8), jsonb_build_object('targetId', 8, 'minutes', 10)),
 jsonb_build_array(
...
(8, '迷雾沼泽', '常年被浓雾笼罩的大沼泽，脚下泥泞不堪行走困难。沼泽深处有幽魂和毒虫潜伏，迷失者不在少数。',
 'TRAINING_ZONE', 26,
 jsonb_build_array(jsonb_build_object('targetId', 7, 'minutes', 10), jsonb_build_object('targetId', 9, 'minutes', 8), jsonb_build_object('targetId', 10, 'minutes', 12)),
 jsonb_build_array(
...
(9, '青云门遗址', '三百年前显赫一时的青云门，在一场突如其来的劫难中覆灭。如今只剩残垣断壁和游荡的怨灵，但也有遗宝留存的传言。',
 'HIDDEN_ZONE', 28,
 jsonb_build_array(jsonb_build_object('targetId', 3, 'minutes', 15), jsonb_build_object('targetId', 8, 'minutes', 8)),
 jsonb_build_array(
...
(10, '古道驿站', '官道旁的简陋驿站，供通往南方的修士和客商打尖过夜。旁边有个小坊市，可以买到一些基础物资。',
 'SAFE_TOWN', 30,
 jsonb_build_array(jsonb_build_object('targetId', 6, 'minutes', 20), jsonb_build_object('targetId', 8, 'minutes', 12), jsonb_build_object('targetId', 11, 'minutes', 15)),
 jsonb_build_array(
...
(11, '飞云城', '南方第一雄城，城墙上法阵流转、灵光隐隐。城内有法宝阁、丹坊、灵兽铺等各类修士设施。',
 'SAFE_TOWN', 35,
 jsonb_build_array(jsonb_build_object('targetId', 10, 'minutes', 15), jsonb_build_object('targetId', 12, 'minutes', 8), jsonb_build_object('targetId', 14, 'minutes', 20), jsonb_build_object('targetId', 18, 'minutes', 25)),
 jsonb_build_array(
...
(12, '万兽山', '妖兽密集的深山老林，据传山中盘踞着数百种不同妖兽，常年兽吼声震天。猎兽修士的天堂和地狱。',
 'TRAINING_ZONE', 38,
 jsonb_build_array(jsonb_build_object('targetId', 11, 'minutes', 8), jsonb_build_object('targetId', 13, 'minutes', 12), jsonb_build_object('targetId', 15, 'minutes', 15)),
 jsonb_build_array(
...
(13, '幽冥谷', '谷中终年不见天日，阴气浓重到在空中凝成雾滴。幽魂在谷中徘徊不去，偶尔能听到凄厉的哭叫声。',
 'TRAINING_ZONE', 42,
 jsonb_build_array(jsonb_build_object('targetId', 12, 'minutes', 12), jsonb_build_object('targetId', 17, 'minutes', 10), jsonb_build_object('targetId', 16, 'minutes', 15)),
 jsonb_build_array(
...
(14, '灵虚洞天', '传说上古大能灵虚真人开辟的洞天福地，入口隐藏在瀑布之后。内有真人亲笔所留的道经和灵药。',
 'HIDDEN_ZONE', 40,
 jsonb_build_array(jsonb_build_object('targetId', 11, 'minutes', 20), jsonb_build_object('targetId', 16, 'minutes', 12)),
 jsonb_build_array(
...
(15, '天剑宗遗址', '以剑道闻名天下的天剑宗，在千年前一场剑劫中覆灭。残存的剑意仍在空中飘荡，进入者须以自己的剑心抗衡。',
 'TRAINING_ZONE', 45,
 jsonb_build_array(jsonb_build_object('targetId', 12, 'minutes', 15), jsonb_build_object('targetId', 17, 'minutes', 12), jsonb_build_object('targetId', 16, 'minutes', 10)),
 jsonb_build_array(
...
(16, '焚天岭', '活火山脉，山腰以上寸草不生，只有流动的岩浆和硫磺气。山中有火属性天材地宝，但也潜伏着火系妖兽。',
 'TRAINING_ZONE', 48,
 jsonb_build_array(jsonb_build_object('targetId', 13, 'minutes', 15), jsonb_build_object('targetId', 15, 'minutes', 10), jsonb_build_object('targetId', 17, 'minutes', 8), jsonb_build_object('targetId', 14, 'minutes', 12)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='九穗禾'), 'weight', 30),
...
(17, '归墟海外围', '传说万物终归之处的大海边缘，海面上浮沉着远古时代的残骸。海风带着浓重的灵气和危险。',
 'TRAINING_ZONE', 50,
 jsonb_build_array(jsonb_build_object('targetId', 13, 'minutes', 10), jsonb_build_object('targetId', 15, 'minutes', 12), jsonb_build_object('targetId', 16, 'minutes', 8), jsonb_build_object('targetId', 20, 'minutes', 15), jsonb_build_object('targetId', 25, 'minutes', 18)),
 jsonb_build_array(
...
(18, '玲珑坊', '以炼器闻名的城池，满街都是法宝铺和锻造炉的叮当声。城内最大的玲珑阁，据说收藏着数件仙器。',
 'SAFE_TOWN', 50,
 jsonb_build_array(jsonb_build_object('targetId', 11, 'minutes', 25), jsonb_build_object('targetId', 20, 'minutes', 10), jsonb_build_object('targetId', 23, 'minutes', 18)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='曼陀罗'), 'weight', 20),
...
(19, '魔王岭', '据说是上古一位魔王的陨落之地，魔气至今未散。山岭上的草木都呈诡异的黑紫色，妖兽也变得异常凶悍。',
 'TRAINING_ZONE', 53,
 jsonb_build_array(jsonb_build_object('targetId', 20, 'minutes', 10), jsonb_build_object('targetId', 25, 'minutes', 12), jsonb_build_object('targetId', 21, 'minutes', 5)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='还魂草种子'), 'weight', 30),
...
(20, '天机阁外山', '天机阁的入口山脉，整座山布满了机关和阵法。天机阁以推演天机和炼制奇宝闻名。',
 'TRAINING_ZONE', 55,
 jsonb_build_array(jsonb_build_object('targetId', 17, 'minutes', 15), jsonb_build_object('targetId', 18, 'minutes', 10), jsonb_build_object('targetId', 19, 'minutes', 10), jsonb_build_object('targetId', 23, 'minutes', 12), jsonb_build_object('targetId', 21, 'minutes', 8)),
 jsonb_build_array(
...
(21, '九幽魔窟·一层', '九幽魔窟的入口层，虽然只是最上层但已让人感觉窒息。洞壁流淌着诡异的暗红色浆体，地底传来隐隐的咆哮声。',
 'HIDDEN_ZONE', 55,
 jsonb_build_array(jsonb_build_object('targetId', 19, 'minutes', 5), jsonb_build_object('targetId', 20, 'minutes', 8)),
 jsonb_build_array(
...
(22, '冰霜冻原', '一望无际的冰原，寒风如刀刮过。冰层之下有无数冰属性妖兽和天材地宝。',
 'TRAINING_ZONE', 55,
 jsonb_build_array(jsonb_build_object('targetId', 23, 'minutes', 15), jsonb_build_object('targetId', 24, 'minutes', 12), jsonb_build_object('targetId', 28, 'minutes', 20)),
 jsonb_build_array(
...
(23, '神木林', '一片古老无比的灵木森林，树冠高耸如天。林中的木灵气浓郁到形成了绿色光雾。',
 'TRAINING_ZONE', 56,
 jsonb_build_array(jsonb_build_object('targetId', 18, 'minutes', 18), jsonb_build_object('targetId', 20, 'minutes', 12), jsonb_build_object('targetId', 22, 'minutes', 15), jsonb_build_object('targetId', 24, 'minutes', 10)),
 jsonb_build_array(
...
(24, '金沙荒漠', '一片无边无际的金色沙海，烈日当空毫不留情。沙漠深处有上古遗迹埋藏，偶尔被风吹出地表一角。',
 'TRAINING_ZONE', 58,
 jsonb_build_array(jsonb_build_object('targetId', 22, 'minutes', 12), jsonb_build_object('targetId', 23, 'minutes', 10), jsonb_build_object('targetId', 25, 'minutes', 8), jsonb_build_object('targetId', 30, 'minutes', 20)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='紫金砂'), 'weight', 30),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='九穗禾'), 'weight', 25),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='太阳花'), 'weight', 20),
...
(25, '暗影沼泽', '比迷雾沼泽更深邃百倍的巨大沼泽，沼泽中暗流涌动，天空永远被黑云遮蔽。',
 'TRAINING_ZONE', 60,
 jsonb_build_array(jsonb_build_object('targetId', 17, 'minutes', 18), jsonb_build_object('targetId', 19, 'minutes', 12), jsonb_build_object('targetId', 24, 'minutes', 8), jsonb_build_object('targetId', 27, 'minutes', 20)),
 jsonb_build_array(
...
(26, '太虚城', '修仙界的中心城之一，城中悬浮着无数仙岛，需乘坐仙鹤或御剑才能往来。各大宗门在此设有分舵。',
 'SAFE_TOWN', 65,
 jsonb_build_array(jsonb_build_object('targetId', 27, 'minutes', 10), jsonb_build_object('targetId', 28, 'minutes', 12), jsonb_build_object('targetId', 42, 'minutes', 30)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='曼陀罗'), 'weight', 20),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='七星草'), 'weight', 20),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='何首乌'), 'weight', 20),
...
(27, '不周山', '传说中支撑天地的神山，山体断裂处倾泻出混沌气息。山中危机四伏，但也充满机缘。',
 'TRAINING_ZONE', 68,
 jsonb_build_array(jsonb_build_object('targetId', 26, 'minutes', 10), jsonb_build_object('targetId', 28, 'minutes', 15), jsonb_build_object('targetId', 31, 'minutes', 18), jsonb_build_object('targetId', 29, 'minutes', 12)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='玄晶'), 'weight', 30),
...
(28, '昆仑墟', '昆仑山深处的远古废墟，相传是仙人居住过的宫殿。废墟中阵法仍在此运转，稍有不慎就会触发毁灭性的禁制。',
 'TRAINING_ZONE', 70,
 jsonb_build_array(jsonb_build_object('targetId', 26, 'minutes', 12), jsonb_build_object('targetId', 27, 'minutes', 15), jsonb_build_object('targetId', 29, 'minutes', 10), jsonb_build_object('targetId', 30, 'minutes', 15)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='紫金砂'), 'weight', 30),
...
(29, '封神台', '远古封神一战遗留的破碎神台，神血洒落之处长出了奇异的血红色花朵。封神榜碎片据说在此飘荡。',
 'HIDDEN_ZONE', 72,
 jsonb_build_array(jsonb_build_object('targetId', 27, 'minutes', 12), jsonb_build_object('targetId', 28, 'minutes', 10), jsonb_build_object('targetId', 34, 'minutes', 15)),
 jsonb_build_array(
...
(30, '星河古道', '一条悬浮于星河之中的古路，两侧是璀璨的星海深渊。古道上遍布星兽和星辰碎片。',
 'TRAINING_ZONE', 73,
 jsonb_build_array(jsonb_build_object('targetId', 24, 'minutes', 20), jsonb_build_object('targetId', 28, 'minutes', 15), jsonb_build_object('targetId', 31, 'minutes', 12), jsonb_build_object('targetId', 33, 'minutes', 18)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='玄晶'), 'weight', 30),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='混沌石'), 'weight', 15),
...
(31, '紫府秘境', '传说紫府真人飞升前留下的秘境，紫色灵气凝聚为液，汇成小溪在秘境中流淌。',
 'HIDDEN_ZONE', 75,
 jsonb_build_array(jsonb_build_object('targetId', 27, 'minutes', 18), jsonb_build_object('targetId', 30, 'minutes', 12), jsonb_build_object('targetId', 32, 'minutes', 10)),
 jsonb_build_array(
...
(32, '九天雷池', '九天之巅的一座巨大雷池，亿万道雷霆在池中翻滚跳跃。修炼雷属性法决的修士在此可一日千里——也可能一命呜呼。',
 'TRAINING_ZONE', 76,
 jsonb_build_array(jsonb_build_object('targetId', 31, 'minutes', 10), jsonb_build_object('targetId', 30, 'minutes', 15), jsonb_build_object('targetId', 34, 'minutes', 12), jsonb_build_object('targetId', 36, 'minutes', 18)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='魂玉碎片'), 'weight', 30),
...
(33, '蚩尤血渊', '蚩尤被斩杀之处，鲜血深渗地层千万丈形成了整座血渊。渊底有蚩尤魔兵残片的传说。',
 'HIDDEN_ZONE', 78,
 jsonb_build_array(jsonb_build_object('targetId', 30, 'minutes', 18), jsonb_build_object('targetId', 35, 'minutes', 10), jsonb_build_object('targetId', 38, 'minutes', 15)),
 jsonb_build_array(
...
(34, '太上道场', '传说太上老君曾在凡间讲道的道场，如今已是一片废墟但道韵犹存。在此修炼可获道祖悟道加成。',
 'TRAINING_ZONE', 80,
 jsonb_build_array(jsonb_build_object('targetId', 29, 'minutes', 15), jsonb_build_object('targetId', 32, 'minutes', 12), jsonb_build_object('targetId', 35, 'minutes', 10), jsonb_build_object('targetId', 38, 'minutes', 15), jsonb_build_object('targetId', 39, 'minutes', 8)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='菩提叶'), 'weight', 25),
...
(35, '仙魔战场', '远古仙魔大战的主战场，空间被打得支离破碎至今未愈合。战场上游荡着仙魔残魂和至宝碎片。',
 'TRAINING_ZONE', 82,
 jsonb_build_array(jsonb_build_object('targetId', 33, 'minutes', 10), jsonb_build_object('targetId', 34, 'minutes', 10), jsonb_build_object('targetId', 36, 'minutes', 12), jsonb_build_object('targetId', 37, 'minutes', 15), jsonb_build_object('targetId', 41, 'minutes', 20)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='魂玉碎片'), 'weight', 25),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='混沌石'), 'weight', 20),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='赤炼果'), 'weight', 20),
...
(36, '万妖谷', '无数大妖盘踞的巨型山谷，是妖族的核心领地之一。非强者不敢靠近。',
 'TRAINING_ZONE', 84,
 jsonb_build_array(jsonb_build_object('targetId', 32, 'minutes', 18), jsonb_build_object('targetId', 35, 'minutes', 12), jsonb_build_object('targetId', 37, 'minutes', 10), jsonb_build_object('targetId', 38, 'minutes', 12)),
 jsonb_build_array(
...
(37, '死神沼泽', '连死神都要绕道走的沼泽，据说沼泽最深处有通向冥界的裂缝。沼泽里的水是黑色的，碰一下就会吞噬生命力。',
 'TRAINING_ZONE', 86,
 jsonb_build_array(jsonb_build_object('targetId', 35, 'minutes', 15), jsonb_build_object('targetId', 36, 'minutes', 10), jsonb_build_object('targetId', 41, 'minutes', 12), jsonb_build_object('targetId', 40, 'minutes', 8)),
 jsonb_build_array(
...
(38, '悟道山', '一座充满道韵的神山，山上的每一块石头上都有天然而成的道纹。在此打坐修炼事半功倍。',
 'TRAINING_ZONE', 88,
 jsonb_build_array(jsonb_build_object('targetId', 33, 'minutes', 15), jsonb_build_object('targetId', 34, 'minutes', 15), jsonb_build_object('targetId', 36, 'minutes', 12), jsonb_build_object('targetId', 39, 'minutes', 10), jsonb_build_object('targetId', 42, 'minutes', 20)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='菩提叶'), 'weight', 30),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='赤炼果'), 'weight', 25),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='灵芝'), 'weight', 20),
...
(39, '大道河', '一条横贯天地的灵河，河水不是水而是液化的大道之力。溯流而上可得道之始，顺流而下可见道之终。',
 'TRAINING_ZONE', 90,
 jsonb_build_array(jsonb_build_object('targetId', 34, 'minutes', 8), jsonb_build_object('targetId', 38, 'minutes', 10), jsonb_build_object('targetId', 40, 'minutes', 12), jsonb_build_object('targetId', 43, 'minutes', 15)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='茯苓'), 'weight', 30),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='菩提叶'), 'weight', 25),
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='赤炼果'), 'weight', 20),
...
(40, '浮世幻境', '一个巨大无比的幻境入口，进去之后面对的是自己内心的破绽和恐惧——但也有突破了它们的机会。',
 'HIDDEN_ZONE', 90,
 jsonb_build_array(jsonb_build_object('targetId', 37, 'minutes', 8), jsonb_build_object('targetId', 39, 'minutes', 12), jsonb_build_object('targetId', 42, 'minutes', 10)),
 jsonb_build_array(
...
(41, '混沌海', '天地未开时混沌原海的残留，进入者需要抵御混沌之气的侵蚀才能存活。但同时混沌中孕育着无限可能。',
 'HIDDEN_ZONE', 95,
 jsonb_build_array(jsonb_build_object('targetId', 35, 'minutes', 20), jsonb_build_object('targetId', 37, 'minutes', 12), jsonb_build_object('targetId', 44, 'minutes', 8), jsonb_build_object('targetId', 43, 'minutes', 12)),
 jsonb_build_array(
...
(42, '飞升台', '修仙界最高的山峰之巅，据说登顶者可感天应道，飞升在望。这里是每个修士的终极目的地。',
 'SAFE_TOWN', 98,
 jsonb_build_array(jsonb_build_object('targetId', 26, 'minutes', 30), jsonb_build_object('targetId', 38, 'minutes', 20), jsonb_build_object('targetId', 40, 'minutes', 10), jsonb_build_object('targetId', 43, 'minutes', 8), jsonb_build_object('targetId', 44, 'minutes', 5)),
 jsonb_build_array(
   jsonb_build_object('templateId', (SELECT id FROM xt_item_template WHERE name='茯苓'), 'weight', 30),
...
(43, '归墟海底', '归墟深处沉没的海底世界，古老的海底城市中封印着被遗忘的远古生物。',
 'TRAINING_ZONE', 98,
 jsonb_build_array(jsonb_build_object('targetId', 39, 'minutes', 15), jsonb_build_object('targetId', 41, 'minutes', 12), jsonb_build_object('targetId', 42, 'minutes', 8), jsonb_build_object('targetId', 44, 'minutes', 10)),
 jsonb_build_array(
...
(44, '仙堺裂缝', '仙界和人界之间的裂缝，凡人误入其中可能瞬间被仙气粉碎——但也可能直接飞升成仙。最高等级的隐藏地图。',
 'HIDDEN_ZONE', 100,
 jsonb_build_array(jsonb_build_object('targetId', 41, 'minutes', 8), jsonb_build_object('targetId', 42, 'minutes', 5), jsonb_build_object('targetId', 43, 'minutes', 10)),
 jsonb_build_array(
