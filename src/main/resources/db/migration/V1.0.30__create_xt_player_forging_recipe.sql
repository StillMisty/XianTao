...
CREATE
    TABLE
        xt_player_forging_recipe(
            ...
            CONSTRAINT uq_user_forging_recipe UNIQUE(
                user_id,
                blueprint_template_id
            ),
            CONSTRAINT fk_player_forging_recipe_user FOREIGN KEY(user_id) REFERENCES xt_user(id) ON
            DELETE
                CASCADE,
...
CREATE
    INDEX idx_player_forging_recipe_user_id ON
    xt_player_forging_recipe(user_id);
...
COMMENT ON
COLUMN xt_player_forging_recipe.user_id IS '用户ID';

COMMENT ON
COLUMN xt_player_forging_recipe.blueprint_template_id IS '锻造图纸模板ID';

COMMENT ON
COLUMN xt_player_forging_recipe.equipment_template_id IS '产出装备模板ID';

COMMENT ON
COLUMN xt_player_forging_recipe.learn_time IS '学习时间';
