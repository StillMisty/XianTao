...
CREATE
    TABLE
        world_event(
            ...
            CONSTRAINT chk_world_event_count CHECK(
...
CREATE
    INDEX idx_world_event_active ON
    world_event(
        start_time,
        end_time
    )
...
CREATE
    INDEX idx_world_event_region ON
    world_event(
        SCOPE,
        region_map_node_id
    )
...
CREATE
    INDEX idx_world_event_chain ON
    world_event(parent_event_id)
WHERE
...
COMMENT ON
COLUMN world_event.category IS '事件类别：ECONOMIC(经济) / ENVIRONMENTAL(环境) / NARRATIVE(叙事) / PARTICIPATORY(参与)';

COMMENT ON
COLUMN world_event.scope IS '作用域：GLOBAL(全体) / REGIONAL(区域)';

COMMENT ON
COLUMN world_event.region_map_node_id IS '区域事件绑定的地图节点';
...
COMMENT ON
COLUMN world_event.participation_effects IS '参与奖励效果 JSONB';

COMMENT ON
COLUMN world_event.parent_event_id IS '父事件ID，用于事件链';

COMMENT ON
COLUMN world_event.chain_order IS '事件链中的顺序';
