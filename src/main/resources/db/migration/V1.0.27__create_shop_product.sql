...
CREATE
    INDEX idx_shop_product_npc ON
    shop_product(shop_npc_id);
...
COMMENT ON
COLUMN shop_product.min_price IS '最低售价';

COMMENT ON
COLUMN shop_product.max_price IS '最高售价';
...
COMMENT ON
COLUMN shop_product.current_stock IS '当前库存（会随供需浮动）';

COMMENT ON
COLUMN shop_product.last_sale_time IS '最后一次交易时间（用于懒调价/懒补货计算）';

COMMENT ON
COLUMN shop_product.version IS '乐观锁版本号';
