...
所有异步场景的扁平枚举（category 级别，具体事件名由 DB `xt_event_type.code` 注册）：

- `TRAVEL_ARRIVED` — 到达目的地
- `TRAVEL_EVENT` — 旅行子事件（含 NUMERIC / CHOICE）
...
- `DYING_RECOVERED`
- `LEVEL_UP`
- `WORLD_EVENT` — 世界事件通知（含地灵事件效果）
- `WORLD_EVENT_PARTICIPATION` — 玩家参与世界事件结果
- `DUNGEON_ENTER` — 进入秘境 / 推进区域
- `DUNGEON_EXPLORE` — 探索 POI 子事件
- `DUNGEON_HIDDEN` — 秘境隐藏事件
- `DUNGEON_COMPLETE` — 秘境通关结算
...
| LEVEL_UP | `━━━ 突破 ━━━` |
| WORLD_EVENT | `━━━ 世界事件 ━━━` |
| WORLD_EVENT_PARTICIPATION | `━━━ 世界事件 ━━━` |
| DUNGEON_* | `━━━ 秘境探索 ━━━` |
...
ALTER TABLE xt_user ADD COLUMN activity_type       VARCHAR(16)
    CHECK (activity_type IN ('TRAVEL', 'TRAINING', 'BOUNTY', 'DUNGEON'));
...
| 秘境 | ❌ 手动探索 | "秘境探索"完成 POI，"秘境继续"推进区域 | 回合制逐步推进 |
...
| 方差 | 极高 — 低概率、大效果 |
| 奖励来源 | 专属事件奖励表 |
| 效果类型 | 百分比优先：ADD_EXP_PERCENT（2-8%升级经验）、TAKE_DAMAGE_PERCENT（5-15%最大生命） |
| 叙事 | "你意外发现…" |
...
**模板变量规则**：每个 `xt_activity_event.params.effects` 中 handler 返回的 key 必须覆盖 `xt_event_type.description` 中所有 `{{key}}` 模板变量。遗漏的占位符会被 `NotificationAppender.renderTemplate` 渲染为 `？`。
...
**旅行 CHOICE 事件**：可选交互 —— 玩家用「选 A/B/C」指令决定走向。触发时暂停自动流程，等待玩家输入后执行对应的 effects。旅行中的 CHOICE 事件示例：
...
CHOICE 事件的效果使用与 NUMERIC 相同的 12 种 effect type，options 中每条分支独立配置 effects 数组。渲染时 `NotificationAppender` 为 CHOICE 事件追加选项编号和提示文本。
...
| 秘境隐藏 | 探索 POI 时，checkHiddenEvents | DungeonEventCompleter |
...
事件产出入口：
  resolveState()    → 旅行到达（NUMERIC + CHOICE 统一池）、HP恢复、buff过期、DYING恢复...
  TrainingService   → 历练结算（统一事件循环：COMBAT + NUMERIC）、历练中断
  BountyService     → 悬赏领奖
  DungeonService    → 秘境进入、探索POI、推进区域、通关结算（DungeonEventCompleter）
  各活动的 completer → 隐藏事件检查、完成叙事产出
  CHOICE 事件        → 触发后暂停自动流程等待「选 A/B/C」指令，ChoiceExecutor 执行对应 effects
...
| DUNGEON 事件 | 进入/推进/通关直接写 GameEvent，探索 POI 用 SubEventSelector | 固定叙事保证仪式感，子事件保持随机惊喜 |
| DUNGEON 隐藏事件 | 绑定 dungeon_id 为 owner_id，触发后记录 xt_hidden_completion | 复用现有隐藏事件机制 |
