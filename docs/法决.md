...
| `formula` | VARCHAR(128) | 伤害/治疗公式，变量: `attack`/`defense`/`str`/`con`/`agi`/`wis`，支持 `+`/`-`/`*`/`/` 和括号以及小数 |
...
> **伤害公式说明**：`formula` 使用递归下降表达式解析器（`DamageCalculator.evaluateFormula`），支持小数点。可用变量：`attack`（攻击）、`defense`（防御）、`wis`（悟性）、`str`（力道）、`con`（根骨）、`agi`（身法）。支持运算：`+`、`-`、`*`、`/`、括号 `()`。例：`"wis*3+20"`、`"attack*1.5"`。
...
| `EXECUTE` | 斩杀（低血量额外伤害） | `[{"type":"EXECUTE","formula":"attack*3.0","chance":0.3,"target":"single"}]` |
| `LIFESTEAL` | 吸血 | `[{"type":"LIFESTEAL","value":15,"target":"single"}]` |
| `AOE_DAMAGE` | 群体伤害 | `[{"type":"AOE_DAMAGE","formula":"attack*1.2","target":"aoe"}]` |
...
| `DOT` | 持续伤害 | `[{"type":"DOT","value":15,"duration":3,"maxStacks":3,"target":"single"}]` |
