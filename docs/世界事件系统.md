...
| type | 参数 | 说明 |
|------|------|------|
| `ADD_EXP_PERCENT` | `percent` (1=N%) | 升级所需经验的百分比 |
...
地灵事件保持独立的触发机制（对话懒生成、时间门控、不持久化），但接入 SubEventEffect 效果管线，允许部分事件对玩家产生机制影响。

### 7.2 有效果的地灵事件

| FudiEvent | 效果 | 设计意图 |
|-----------|------|----------|
| `WEATHER_RAIN` | ADD_EXP_PERCENT 5% | 灵雨助修炼 |
| `SPIRIT_BUTTERFLY` | ADD_SPIRIT_STONES × 3 | 灵蝶的小礼物 |
| `MYSTERIOUS_VISITOR` | ADD_RANDOM_ITEM (MATERIAL_COMMON) | 访客留下谢礼 |
| `BEAST_BIRTH` | ADD_EXP_PERCENT 8% | 新生命带来灵气 |
| `ENERGY_RESTORE` | HEAL_FLAT × 30 | 灵气自然恢复 |

其余 7 个 FudiEvent 保持纯叙事无效果。
...
flowchart TD
    ...
    H[地灵事件<br/>FudiEvent<br/>对话时懒生成] --> F
```
