...
```
xt_beast_template           ← 灵兽模板配置（孵化配置、技能池、产出物、tags）
  └── id (PK)

xt_item_template (BEAST_EGG) ← 兽卵物品模板
  └── properties.beast_template_id → xt_beast_template.id

xt_beast                     ← 灵兽实体（唯一数据源）
  └── template_id → xt_beast_template.id
...
### 1.2 灵兽模板表 (xt_beast_template)

| 字段                | 类型           | 说明                                           |
|-------------------|--------------|----------------------------------------------|
| `id`              | BIGSERIAL    | PK                                           |
| `name`            | VARCHAR(128) | 灵兽名称（唯一）                                    |
| `grow_time`       | INT          | 孵化时间（小时）                                    |
...
| `user_id`                     | BIGINT       | 所属用户                                     |
| `fudi_id`                     | BIGINT       | 所属福地                                     |
| `template_id`                 | BIGINT       | FK → xt_beast_template(id)，灵兽模板ID        |
...
兽卵 (`xt_item_template`, type=BEAST_EGG) 的 `properties` 简化为引用灵兽模板：
...
所有孵化配置（孵化时间、产出物、技能池）均存储在 `xt_beast_template` 中。

### 1.5 变异特性配置表 (xt_mutation_trait_config)
...
筛选逻辑：灵兽模板 (`xt_beast_template.tags`) 必须**全部包含**所需tags，且品质 >= 最低品质。
...
每种灵兽模板 (`xt_beast_template`) 有 tags，如 `["flying","phoenix"]`。

繁育时：
1. 提取父母双方灵兽模板的 tags（去掉稀有度标签）
2. 合并去重
...
产出物由灵兽模板 (`xt_beast_template.production_items`) 决定，不同品种产出不同药材：
...
每轮产出时，从 `production_items` 中按权重随机选取物品。产出配置通过 `beast.template_id` 查找灵兽模板获得。
...
每个灵兽模板 (`xt_beast_template`) 配置两组技能池：
